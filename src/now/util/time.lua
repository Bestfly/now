---一些时间相关的辅助函数
module("now.util.time",package.seeall)
...
function get_dis(dis)
    ...
    return math.ceil(num * _dis[kind])
end

---根据距离的间隔字符串
--@param #int dis 间隔的秒数
--@param #string kind 计算的类型w=周，d=天，h=小时，m=分钟，s=秒
function dis_str(dis, kind)
...
function date_diff(date1, date2, kind)
	local time1 = get_time(date1)
	local time2 = get_time(date2)
	if time1 < time2 then
		return -dis_str(time1 - time2, kind)
	else
		return dis_str(time1 - time2, kind)
	end
end
...
---日期增加
function date_add(date, dis)
	local time = get_time(date)
	time = time + get_dis(dis)
